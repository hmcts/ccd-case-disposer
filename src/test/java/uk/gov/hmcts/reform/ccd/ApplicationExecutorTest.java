...
class ApplicationExecutorTest {
    ...
    void shouldContinueDeletionsAfterLogAndAuditError(CapturedOutput output) {
        ...
        doAnswer(invocation -> {
            ...
            if (TestData.DELETABLE_CASE_DATA_WITH_PAST_TTL.equals(caseData)) {
    ...
    void shouldWaitForAllTasksToComplete() throws Exception {
        ...
        when(caseCollectorService.getDeletableCases(List.of(TestData.DELETABLE_CASE_TYPE))).thenReturn(cases);
        givenNoSimulatedCases();
    ...
    void shouldWaitForRemainingTasksAfterUnexpectedTaskFailure() throws Exception {
        ...
        verify(caseDeletionService).deleteCaseData(TestData.DELETABLE_CASE_DATA_WITH_PAST_TTL);
        verify(caseDeletionService).deleteCaseData(TestData.DELETABLE_CASE_DATA4_WITH_PAST_TTL);
        verify(processedCasesRecordHolder, never()).addProcessedCase(TestData.DELETABLE_CASE_DATA_WITH_PAST_TTL);
        verify(processedCasesRecordHolder).addProcessedCase(TestData.DELETABLE_CASE_DATA4_WITH_PAST_TTL);
        verify(caseDeletionLoggingService).logCases();
    }
    ...
    private void givenNoSimulatedCases() {
        when(caseCollectorService.getDeletableCases(
            List.of(TestData.DELETABLE_CASE_TYPE_SIMULATION))).thenReturn(Set.of());
    }
}
