...
class CaseCollectorServiceTest {
    ...
    void getDeletableCases_ReturnsAll_WhenAllLinksExpired() {
        ...
        given(caseLinkRepository.findByCaseIdInOrLinkedCaseIdIn(List.of("MyType", "AnotherType")))
    ...
    void getDeletableCases_ExcludesCasesLinkedToNonExpired() {
        ...
        given(caseDataRepository.findExpiredCases(anyList())).willReturn(expired);
        given(caseLinkRepository.findByCaseIdInOrLinkedCaseIdIn(List.of("TYPE"))).willReturn(List.of(link(1, 99)));
    ...
    void getDeletableCases_TraversesTransitively() {
        ...
        given(caseDataRepository.findExpiredCases(anyList())).willReturn(expired);
        given(caseLinkRepository.findByCaseIdInOrLinkedCaseIdIn(List.of("TYPE")))
            .willReturn(List.of(
                link(1, 2),
                link(2, 99),
                link(3, 4),
    ...
    void getDeletableCases_HandlesSelfLinks() {
        ...
        given(caseLinkRepository.findByCaseIdInOrLinkedCaseIdIn(List.of("TYPE"))).willReturn(List.of(link(1, 1)));
        Set<CaseData> result = service.getDeletableCases(List.of("TYPE"));
    ...
    void getDeletableCases_HandlesManyLinks() {
        given(caseDataRepository.findExpiredCases(anyList())).willReturn(List.of(entity(1), entity(2), entity(3)));
        given(caseLinkRepository.findByCaseIdInOrLinkedCaseIdIn(List.of("TYPE"))).willReturn(List.of(
            link(1, 2),
            link(1, 3),
            link(1, 4),
            link(1, 5)
