...
public class CaseDeletionService {
    ...
    boolean deleteCaseLinks(final CaseData caseData) {
        try {
            int deletedCaseLinks = 0;
            log.info("About to delete linked case reference:: {}", caseData.getReference());
            deletedCaseLinks = caseLinkRepository.deleteCaseLinks(caseData.getId());
            log.info("Deleted linked case reference:: {} and Total deleted linked cases : {} ",
                         caseData.getReference(), deletedCaseLinks);
            return true;
